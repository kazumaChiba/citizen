(function( $ ) {
  ...
  Loadie.finish = function(dom) {
    var loadie = $('#loadie-' + dom.data('loadie-id'), dom);
        ...
        //$('.logo_loader').delay(500).fadeOut(800);
        //$('.loading').delay(1000).fadeOut(800);
        
         TweenMax.to('.loading',1,{delay:2,alpha: 0,ease:Quart.easeOut});
         TweenMax.to('.loading',.5,{alpha: 0});
         TweenMax.to('.loading',0,{left:'-100%',alpha: 0,ease:Quart.easeOut,onComplete: complete});
